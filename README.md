...
> **Разработка и деплой:** изменения только в порядке **локально → Git → сервер**.  
> Пуш в Git и деплой на сервер — **только по явной команде владельца проекта**.  
...
В отчёте «Последняя проверка» поле **плановое окончание** — оценка по числу оставшихся URL и интервалу троттлинга, а не «старт + 5 часов».
...
Новые пользователи отправляют заявку на доступ; администратор одобряет или отклоняет её кнопками в Telegram.
